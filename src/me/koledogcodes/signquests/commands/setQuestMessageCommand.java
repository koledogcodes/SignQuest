...
public class setQuestMessageCommand implements CommandExecutor {
	...
	public setQuestMessageCommand(SignQuest i) {
		plugin = i;
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (args.length == 3){
			if (args[1].equalsIgnoreCase("remove")){
				try {
					handler.removeQuestMessage(sender, args[0].toLowerCase(), Integer.parseInt(args[2]));
				}
				catch (Exception e){
		...
		if (args.length >= 3){
			...
			if (args[1].equalsIgnoreCase("add")){
				...
				handler.addQuestMessage(sender, args[0].toLowerCase(), handler.colorConvert(builder.get(sender).toString()));
			}
			else {
				ChatUtili.sendTranslatedMessage(sender, "&cInvalid arguments!");
			}
